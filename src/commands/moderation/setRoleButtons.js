...
module.exports = {
	...
	callback: async (client, interaction) => {
		...
		try {
			...
			await channel.send({
				content:
					"Rol almak ya da rol kaldırmak için butona tıklayabilirsin",
				components: [row],
			})
			await interaction.editReply(
				`${channel.toString()} kanalına rol butonları gönderildi`
			)
		} catch (error) {
			console.log(`There is error at giveRole.\nError: ${error}`)
			await interaction.editReply(`İşlem gerçekleştirilemedi!`)
		}
